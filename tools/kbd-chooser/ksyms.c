...
int set_charset(const char *charset) {
	...
	di_logf (PROGNAME ":unknown charset %s - ignoring charset request\n", charset);
	return 1;
}
	...
	if (prefer_unicode) {
		...
		for (i = 0; i < 256 - 160; i++)
			if (!strcmp(s, latin1_syms[i].name)) {
				di_logf (PROGNAME ": assuming iso-8859-1 %s\n", s);
				return K(KT_LATIN, 160 + i);
			}

		for (i = 0; i < 256 - 160; i++)
			if (!strcmp(s, iso_8859_15_syms[i].name)) {
				di_logf (PROGNAME ": assuming iso-8859-15 %s\n", s);
				return K(KT_LATIN, 160 + i);
			}

		for (i = 0; i < 256 - 160; i++)
			if (!strcmp(s, latin2_syms[i].name)) {
				di_logf (PROGNAME ": assuming iso-8859-2 %s\n", s);
				return K(KT_LATIN, 160 + i);
			}
		...
		for (i = 0; i < 256 - 160; i++)
			if (!strcmp(s, latin3_syms[i].name)) {
				di_logf (PROGNAME ": assuming iso-8859-3 %s\n", s);
				return K(KT_LATIN, 160 + i);
			}
#endif
		...
		for (i = 0; i < 256 - 160; i++)
			if (!strcmp(s, latin4_syms[i].name)) {
				di_logf (PROGNAME ": assuming iso-8859-4 %s\n", s);
				return K(KT_LATIN, 160 + i);
			}
#endif
	}

	di_logf (PROGNAME ": unknown keysym '%s'\n", s);
...
add_capslock(int code)
{
	...
	di_logf (PROGNAME ": plus before %s ignored\n", p);
	return code;
}
